...
class DataProcessor:
    ...
    def train_prophet_model(self, df):
        ...
        # Add floor and capacity columns
        df['floor'] = 0
        # Set capacity to a more realistic value (max observed value + 20%)
        max_observed = df['y'].max()
        df['cap'] = max_observed * 1.2  # 최대 관측값의 120%로 설정
        
        model = Prophet(
            holidays=events,
            growth='logistic',  # Use logistic growth
            changepoint_prior_scale=0.05,  # 기본값
            seasonality_prior_scale=10.0,  # 기본값
            holidays_prior_scale=10.0,  # 기본값
    ...
    def make_predictions(self, model, periods=560):
        ...
        future['floor'] = 0  # Set floor value for predictions
        future['cap'] = model.history['cap'].iloc[0]  # Use the same capacity as training data
    ...
    def predict_all_items(self, time_index=1799):
        ...
        for index, rows in items_to_process.iterrows():
            try:
                ...
                future['floor'] = 0  # Set floor value for predictions
                future['cap'] = m.history['cap'].iloc[0]  # Use the same capacity as training data
